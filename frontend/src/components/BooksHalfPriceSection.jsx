...
export const BooksHalfPriceSection = () => {
  const { books, loading, error } = useBooks();

  const navigate = useNavigate();
  const list = Array.isArray(books) ? books.slice(0, 8) : [];
  return (
    <section className="py-20 bg-white">
      <div className="max-w-9xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-3xl font-serif text-gray-900">Books Half Price</h2>
        ...
        <Swiper
          ...
          className="books-half-price-swiper"
        >
          {list.map((book) => (
            <SwiperSlide key={book.id}>
              <div className="group relative flex flex-col items-center bg-white rounded-lg p-4 transition-all duration-300 hover:shadow-lg">
                <div className="relative w-full overflow-hidden rounded-lg">
                  ...
                  <span className="absolute top-2 left-2 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded">-50%</span>
                ...
                <div className="w-full text-center mt-4">
                  ...
                  <p className="text-lg font-bold text-green-700 mt-2">{book.price === null || book.price === undefined || book.price === '' ? '—' : `$${book.price}`}</p>
                </div>
              </div>
            </SwiperSlide>
          ))}
